...
from tinyzero_trl.utils import is_multimodal_model
...
class GRPOTrainer(Trainer):
    ...
    _tag_names = ["grpo-multimodal"]
    is_multimodal = False  # Add multimodal model support
    ...
    def __init__(
        ...
        # Models
        # Add multimodal model support
        self.is_multimodal = is_multimodal_model(model if isinstance(model, str) else model.config._name_or_path)
        # Trained model
        ...
        if isinstance(model, str):
            ...
            if self.is_multimodal:
                model = AutoModelForImageTextToText.from_pretrained(model_id, **model_init_kwargs)
            else:
        ...
        if self.beta == 0.0:
            ...
            # Add multimodal model support
            self.ref_model = model.__class__.from_pretrained(model_id, **model_init_kwargs)
        ...
        # Multi-step
        self.num_iterations = args.num_iterations  # = u in the GRPO paper
            ...
            if self.accelerator.is_main_process:
                ...
                if (
                    ...
                    raise ValueError(
                        ...
                        "value lower than the number of GPUs available on your machine¡ªtypically, reducing it by one "
    ...
    @profiling_decorator
    # Add multimodal model support
    def _get_per_token_logps(self, model, inputs, logits_to_keep):
        # We add 1 to `logits_to_keep` because the last logits of the sequence is later excluded
        # Add multimodal model support
        try:
            logits = model(**inputs, logits_to_keep=logits_to_keep + 1).logits
        except TypeError:
            warnings.warn("logits_to_keep argument isn't supported, so we drop logits ourselves.")
    ...
    def _move_model_to_vllm(self):
        ...
        # Performance of runs during inference was way worse than the performance during training: https://github.com/huggingface/trl/issues/3024
        if self.accelerator.is_main_process:
    ...
    def _generate_and_score_completions(
        ...
        # Add multimodal model support
        if self.is_multimodal:
            if not all(x.get("image", None) for x in inputs):
                raise ValueError("Image is required for every sample in a batch for multimodal models.")
            ...
            prompt_inputs = self.processing_class(
                text=prompts_text, return_tensors="pt", padding=True, padding_side="left", add_special_tokens=False  # Gemma3 processing class has images in the first arguments
            )
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                # prompt individually.
                ordered_set_of_prompts = all_prompts_text[:: self.num_generations]  # Relax the assumption that prompts are unique within a batch
        ...
        with torch.no_grad():
            ...
            if self.num_iterations > 1:
                ...
                old_per_token_logps = self._get_per_token_logps(
                    self.model, ref_inputs, logits_to_keep
                )
            else:
            ...
            if self.beta == 0.0:
                ...
                ref_per_token_logps = self._get_per_token_logps(
                    self.ref_model, ref_inputs, logits_to_keep
                )
            else:
                # Add multimodal model support
        ...
        # Add multimodal model support: Only log for first process
            ...
            if self.accelerator.is_main_process:
                ...
                if self.args.report_to and "tensorboard" in self.args.report_to:
                    ...
                    if tensorboard_callback is None:
                        raise ValueError(
                            "TensorBoardCallback not found. Make sure you are using TensorBoardCallback in your Trainer."
                        )
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        per_token_logps = self._get_per_token_logps( model, infer_inputs, logits_to_keep)
